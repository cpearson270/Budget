...
if (exists(file.path(path_transactions,file_trans.csv))) {
...
#dt_transactions_sum_expense1<- dr_transactions_sum_budget1%>% 
#  group_by(expense_group, expense_group_id) %>% 
#  summarise(totals= sum(sum)) %>% 
#  ungroup() 
...
#dt_transactions_sum_expense2<- dr_transactions_sum_budget2%>% 
#  group_by(expense_group_id) %>% 
# summarise(expense_group = unique(expense_group),
#            totals= sum(sum)) %>% 
#  ungroup() %>% 
...
#          filter(expense_group_id == 300) %>% 
#          select(expense_group_id, expense_group) %>% 
#          mutate(totals = with(dt_transactions_sum_expense2,
  #                               sum(totals[
  #                                 expense_group_id %in% c(0,100,200)]))))
  
#dt_transactions_sum_expense2<- dt_transactions_sum_expense2 %>% 
# add_row(expense_group_id=300,
#          expense_group = "calculated savings",
#          totals = with(dt_transactions_sum_expense2,
#                        sum(totals[expense_group_id %in% c(0,100,200)])))

#savings_calc<-rt_budget %>% 
#filter(expense_group_id == 300) %>% 
#select(expense_group_id, expense_group) %>% 
#mutate(totals = with(dt_transactions_sum_expense2,
#                     sum(totals[expense_group_id %in% c(0,100,200)])))
