...
## Last update: 06/02/2022
    # UNDER CONSTRUCTION #
      # Need to test single month,
      # then create loop to complete calcs for all months
...
ds_trans_budget <- dt_log_budget %>% 
  ...
  select(-c(cum_balance, cum_average))
...
dt_log_budget <- rows_upsert(dt_log_budget,
              ...
              #long term col doesnt exist here, can it be removed earlier in 
                  #Budget tracker code?
                                         #long_term
                                         )),
...
v.write_off <- if (any(ds_trans_budget$diff_flag %in% c(130,230))) {
  dt_log_budget %>% 
