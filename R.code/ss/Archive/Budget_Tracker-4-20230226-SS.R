...
### CATEGORISE transactions if from bank, else skip to SUMMARISE

if (!"category" %in% colnames(df_trans_load)) {
  source("R.code/calc.categorise.R")
  
  u.cat_complete<-readline(prompt = 
                             "Categories Comfired? Continue? (Y/N):") %>% 
    tolower()
  ...
  if (u.cat_complete %in% c("","y")) {
  ## load in categeorisation excel-QC
  } else if (u.cat_complete == 'n') {
  break
  }
}
