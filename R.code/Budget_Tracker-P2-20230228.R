  ...
  if(nrow(ds_trans.missing_cat)>0) {
    ...
    for (i in 1:nMissing) {
      ...
      if(any(rt_budget$budget_group_id==
        ...
        df_new_budget_category$budget_group[i] <- 
          readline(prompt = 
                     "Assign new Budget group (Custom nameor leave blank):")
        ...
        df_new_budget_category$long_term[i] <-
          readline(prompt = 
                     "Is this a long term category? ('Y' or leave blank):")
        
        df_new_budget_category<- df_new_budget_category %>% 
          mutate(across(where(is.character), ~na_if(.,"")))#Replace blank values with 'NA'
      }
...
rmarkdown::render("R.code/Budget Report Template-202205-20220506.Rmd",
